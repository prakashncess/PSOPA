...
x_actual=[400 1400 1600];
y_actual=[750 200 700];
model3=[x_actual;y_actual];
save('Model3.dat','model3','-ascii')
...
for ww=1:2          %ww=1 for without noise or ww=2 for with noise
       ...
       %Starting of PSO algorithm 
                ...
                if ww==1
                    if rhh==1
                        filename=sprintf('model3_point%d_without_noise_fixed_rho_grd.dat',plg);
                    else
                        filename=sprintf('model3_point%d_without_noise_varying_rho_grd.dat',plg);
                    end
                else
                    if rhh==1
                        filename=sprintf('model3_point%d_with_noise_fixed_rho_grd.dat',plg);
                    else
                        filename=sprintf('model3_point%d_with_noise_varying_rho_grd.dat',plg);
                    end
                end
...
function val=constrained11(x)
    ...
    gg=((poly_perim(x1,y1))^2/(4*pi*poly_area(x1,y1)))-2.7;
    val=(max(0,gg))^2;
end
...
function val=constrained12(x)
    ...
    gg=-(((poly_perim(x1,y1))^2/(4*pi*poly_area(x1,y1)))-1.2);
    val=(max(0,gg))^2;
end
