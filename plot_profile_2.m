...
close all
pt_sl=5; nn=40;
yy=importdata('C:\Users\Arka\Desktop\pso_ulti_final\real_data\Residual gravity anomaly of the Noranda Mining District_observed data.bln');
x_obs1=yy(:,1);
data_grv=yy(:,2);
...
data_gv=data1*10^5;
data_gr=data3;
dataa=importdata('real_profile3_point7_locations.dat');
sz_dt=size(dataa);
for cnt=1:sz_dt(1)
    ...
    %msft(cnt)=100*sqrt((1/length(yy)).*sum((data_grv'-10^5*polygrav_arctan(x_obs,z_obs,xv1,yv1,rho_estd(cnt))).^2))./(max(data_grv(:))-min(data_grv(:)));
     %msft(cnt)=100*(1/length(yy)).*sqrt(sum((data_grv'-10^5*polygrav_arctan(x_obs,z_obs,xv1,yv1,rho_estd(cnt))./data_grv').^2));
end
            [ss,ii]=sort(msft);
