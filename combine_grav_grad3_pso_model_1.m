...
%Model for Rectangular Body
x_actual=[400 1000 1800 1800];
y_actual=[800 300 300 800];
model1=[x_actual;y_actual];
save('Model1.dat','model1','-ascii')
...
for ww=1:2          %ww=1 for without noise or ww=2 for with noise
       ...
       %Starting of PSO algorithm 
                ...
                if ww==1
                    if rhh==1
                        filename=sprintf('model1_point%d_without_noise_fixed_rho_grd.dat',plg);
                    else
                        filename=sprintf('model1_point%d_without_noise_varying_rho_grd.dat',plg);
                    end
                else
                    if rhh==1
                        filename=sprintf('model1_point%d_with_noise_fixed_rho_grd.dat',plg);
                    else
                        filename=sprintf('model1_point%d_with_noise_varying_rho_grd.dat',plg);
                    end
                end
...
function val=constrained11(x)
    ...
    gg=((poly_perim(x1,y1))^2/(4*pi*poly_area(x1,y1)))-1.9;
    val=(max(0,gg))^2;
end
